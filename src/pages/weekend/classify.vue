<template>
	<div>
		<div class="classify">
	  	...
	  	<div class="near-scape border-top" v-if="nearScapeInfo.length>5">
	  		<a class="near-scape-item border-rightbottom" v-for="(item, index) in nearScapeInfo" :key="index+'nearScapeInfo'" :href="item.link">{{item.title}}</a>
	  	</div>
	  	<div class="weekend-chosen border-topbottom">
	  		<p class="weekend-chosen-title"><span class="pinterest-titleicon"></span>周末精选</p>
	  		<div class="weekend-chosen-items" v-for="(item, index) in weekendChosenInfo" :key="item+'weekendChosen'">
